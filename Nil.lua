...
local function is_friend(player)
    if not getgenv().SKEGG_CONFIG.SilentAim.FriendCheck then return false end
    return local_player:IsFriendsWith(player.UserId)
end

local function is_in_fov(position)
    if getgenv().SKEGG_CONFIG.SilentAim.FOV == 5000 then return true end
    ...
    return distance <= getgenv().SKEGG_CONFIG.SilentAim.FOV
end

local function get_closest_player_mouse_mode()
    local mouse_position = user_input_service:GetMouseLocation()
    local radius = getgenv().SKEGG_CONFIG.SilentAim.FOV
    ...
    for _, player in players:GetPlayers() do
        ...
        if getgenv().SKEGG_CONFIG.SilentAim.TeamCheck and player.Team and local_player.Team and player.Team == local_player.Team then
            continue
        end
        
        if getgenv().SKEGG_CONFIG.SilentAim.FriendCheck and not is_friend(player) then
            continue
        end
        
        if getgenv().SKEGG_CONFIG.SilentAim.AirOnly then
        ...
        if table.find(getgenv().SKEGG_CONFIG.SilentAim.Blacklist, player.Name) then
            continue
        end
        
        local character = player.Character
        local aim_part = character:FindFirstChild(getgenv().SKEGG_CONFIG.SilentAim.AimPart)
        if not aim_part then aim_part = character:FindFirstChild("HumanoidRootPart") end
        
        local screen_position = world_to_screen(aim_part.Position)
        if not screen_position.on_screen then continue end
        
        if not is_in_fov(aim_part.Position) then continue end
        
        local distance = (mouse_position - screen_position.position).Magnitude
        
        if getgenv().SKEGG_CONFIG.SilentAim.PriorityOnly then
            if table.find(getgenv().SKEGG_CONFIG.SilentAim.PriorityList, player.Name) and distance < closest_distance then
...
local function get_closest_player_vector_mode()
    local camera_position = camera.CFrame.Position
    local camera_look_vector = camera.CFrame.LookVector * 1000
    local target_position = camera_position + camera_look_vector
    ...
    for _, player in players:GetPlayers() do
        ...
        local player_position = aim_part.Position
        
        if not is_in_fov(player_position) then continue end
...
local function create_fov_circle()
    if script_table.fov_circle then script_table.fov_circle:Remove() end
    script_table.fov_circle = Drawing.new("Circle")
    script_table.fov_circle.Visible = getgenv().SKEGG_CONFIG.SilentAim.ShowFOV
    script_table.fov_circle.Radius = getgenv().SKEGG_CONFIG.SilentAim.FOV
    script_table.fov_circle.Color = getgenv().SKEGG_CONFIG.SilentAim.FOVColor
    script_table.fov_circle.Thickness = getgenv().SKEGG_CONFIG.SilentAim.FOVThickness
    script_table.fov_circle.Filled = false
    script_table.fov_circle.Position = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y / 2)
end

create_fov_circle()

local function auto_activate_tool()
    if not getgenv().SKEGG_CONFIG.AutoFire.Enabled then return end
    if not getgenv().SKEGG_CONFIG.AutoFire.AutoActivateTool then return end
    
    local current_time = tick()
    local wait_time = 1 / getgenv().SKEGG_CONFIG.AutoFire.FireRate
    
    if current_time - script_table.last_fire_time >= wait_time then
        ...
        if tool then
            tool:Activate()
            script_table.last_fire_time = current_time
        end
    end
end

local function setup_auto_fire()
    if script_table.auto_fire_connection then
        script_table.auto_fire_connection:Disconnect()
        script_table.auto_fire_connection = nil
    end
    
    if getgenv().SKEGG_CONFIG.AutoFire.Enabled then
        script_table.auto_fire_connection = run_service.Heartbeat:Conne
        (function()
            auto_activate_tool()
        end)
    end
...
run_service.RenderStepped:Connect(function()
    script_table.mouse_position = user_input_service:GetMouseLocation()
    
    if not getgenv().SKEGG_CONFIG.SilentAim.Enabled then
        script_table.silent_aim_is_targetting = false
        script_table.silent_aim_target = nil
        return
    end
    
    local new_target = nil
    if getgenv().SKEGG_CONFIG.SilentAim.Mode == "Mouse" then
        new_target = get_closest_player_mouse_mode()
    else
        new_target = get_closest_player_vector_mode()
    end
    
    script_table.silent_aim_is_targetting = new_target and true or false
    script_table.silent_aim_target = new_target or nil
    
    if script_table.silent_aim_target and has_character(script_table.silent_aim_target) then
        local character = script_table.silent_aim_target.Character
        local aim_part = character:FindFirstChild(getgenv().SKEGG_CONFIG.SilentAim.AimPart)
        if not aim_part then aim_part = character:FindFirstChild("HumanoidRootPart") end
        
        local base_position = aim_part.Position
        local velocity = aim_part.Velocity or Vector3.new(0, 0, 0)
        
        local prediction = getgenv().SKEGG_CONFIG.SilentAim.Prediction
        local predicted_position = base_position + (velocity * prediction)
        
        script_table.aim_position = predicted_position
    end
    
    if script_table.fov_circle then
        script_table.fov_circle.Visible = getgenv().SKEGG_CONFIG.SilentAim.ShowFOV
        script_table.fov_circle.Radius = getgenv().SKEGG_CONFIG.SilentAim.FOV
        script_table.fov_circle.Color = getgenv().SKEGG_CONFIG.SilentAim.FOVColor
        script_table.fov_circle.Thickness = getgenv().SKEGG_CONFIG.SilentAim.FOVThickness
...
__namecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and getgenv().SKEGG_CONFIG.SilentAim.Enabled and script_table.silent_aim_is_targetting and script_table.silent_aim_target and self == workspace and method == "Raycast" then
        local origin = args[1]
        args[2] = get_direction(origin, script_table.aim_position)
...
local Window = library:New({
    Name = "skegg-agent",
    Accent = Color3.fromRGB(133, 87, 242),
    sizeX = 500,
    sizeY = 470
})
...
local ListsPage = Window:Page({Name = "Lists"})
local AutoFirePage = Window:Page({Name = "Auto Fire"})

local SilentAimSection = MainPage:Section({Name = "Silent Aim", Side = "Left", Max = 6})
local VisualsSection = MainPage:Section({Name = "Visuals", Side = "Right", Max = 4})

SilentAimSection:Toggle({
    Name = "Enable Silent Aim",
    Default = false,
    Pointer = "silentaim_enable",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.Enabled = state
    end
})

SilentAimSection:Dropdown({
    Name = "Silent Aim Mode",
    Options = {"Mouse", "Vector"},
    Default = "Mouse",
    Pointer = "silentaim_mode",
    callback = function(mode)
        getgenv().SKEGG_CONFIG.SilentAim.Mode = mode
    end
})
...
SilentAimSection:Dropdown({
    ...
    Default = "Head",
    Pointer = "silentaim_part",
    callback = function(part)
        getgenv().SKEGG_CONFIG.SilentAim.AimPart = part
    end
})
...
SilentAimSection:Slider({
    ...
    Maximum = 5000,
    Default = 5000,
    Pointer = "silentaim_fov",
    callback = function(value)
        getgenv().SKEGG_CONFIG.SilentAim.FOV = value
    end
})
...
SilentAimSection:Slider({
    ...
    Default = 0.1,
    Decimals = 0.01,
    Pointer = "silentaim_smoothness",
    callback = function(value)
        getgenv().SKEGG_CONFIG.SilentAim.Smoothness = value
    end
})
...
SilentAimSection:Slider({
    ...
    Default = 0.165,
    Decimals = 0.001,
    Pointer = "silentaim_prediction",
    callback = function(value)
        getgenv().SKEGG_CONFIG.SilentAim.Prediction = value
    end
})
...
VisualsSection:Toggle({
    Name = "Show FOV",
    Default = true,
    Pointer = "silentaim_showfov",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.ShowFOV = state
    end
}):Colorpicker({
    Name = "FOV Color",
    Default = Color3.fromRGB(255, 255, 255),
    Pointer = "visuals_fovcolor",
    callback = function(color)
        getgenv().SKEGG_CONFIG.SilentAim.FOVColor = color
    end
})
...
VisualsSection:Slider({
    ...
    Maximum = 5,
    Default = 1,
    Pointer = "visuals_fovthickness",
    callback = function(value)
        getgenv().SKEGG_CONFIG.SilentAim.FOVThickness = value
    end
})

local SettingsSection = SettingsPage:Section({Name = "Settings", Side = "Left", Max = 6})
local FiltersSection = SettingsPage:Section({Name = "Filters", Side = "Right", Max = 4})

SettingsSection:Toggle({
    Name = "Team Check",
    Default = true,
    Pointer = "settings_teamcheck",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.TeamCheck = state
    end
})
...
SettingsSection:Toggle({
    Name = "Friend Only",
    Default = false,
    Pointer = "settings_friendonly",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.FriendCheck = state
    end
})
...
SettingsSection:Toggle({
    Name = "Air Only",
    Default = false,
    Pointer = "settings_aironly",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.AirOnly = state
    end
})
...
SettingsSection:Toggle({
    Name = "Priority Only",
    Default = false,
    Pointer = "settings_priorityonly",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.PriorityOnly = state
    end
})
...
SettingsSection:Toggle({
    Name = "Only Visible",
    Default = false,
    Pointer = "settings_onlyvisible",
    callback = function(state)
        getgenv().SKEGG_CONFIG.SilentAim.OnlyVisible = state
    end
})

local PriorityListSection = ListsPage:Section({Name = "Priority List", Side = "Left", Max = 4})
local WhitelistSection = ListsPage:Section({Name = "Whitelist", Side = "Right", Max = 4})
local BlacklistSection = ListsPage:Section({Name = "Blacklist", Side = "Left", Max = 4})

PriorityListSection:Box({
    Name = "Add Priority",
    Placeholder = "Player name",
    Pointer = "priority_add",
    callback = function(text)
        if text and text ~= "" then
            table.insert(getgenv().SKEGG_CONFIG.SilentAim.PriorityList, text)
            print("Added to Priority List:", text)
        end
    end
})

PriorityListSection:Button({
    Name = "Clear Priority List",
    callback = function()
        getgenv().SKEGG_CONFIG.SilentAim.PriorityList = {}
...
WhitelistSection:Box({
    Name = "Add Whitelist",
    Placeholder = "Player name",
    Pointer = "whitelist_add",
    callback = function(text)
        if text and text ~= "" then
            table.insert(getgenv().SKEGG_CONFIG.SilentAim.Whitelist, text)
            print("Added to Whitelist:", text)
        end
    end
})
...
WhitelistSection:Button({
    ...
    callback = function()
        getgenv().SKEGG_CONFIG.SilentAim.Whitelist = {}
        print("Whitelist cleared!")
    end
})

BlacklistSection:Box({
    Name = "Add Blacklist",
    Placeholder = "Player name",
    Pointer = "blacklist_add",
    callback = function(text)
        if text and text ~= "" then
            table.insert(getgenv().SKEGG_CONFIG.SilentAim.Blacklist, text)
            print("Added to Blacklist:", text)
        end
    end
})
...
BlacklistSection:Button({
    ...
    callback = function()
        getgenv().SKEGG_CONFIG.SilentAim.Blacklist = {}
        print("Blacklist cleared!")
    end
})

local AutoFireMainSection = AutoFirePage:Section({Name = "Auto Fire", Side = "Left", Max = 3})
local FireRateSection = AutoFirePage:Section({Name = "Settings", Side = "Right", Max = 3})

AutoFireMainSection:Toggle({
    Name = "Enable Auto Fire",
    Default = false,
    Pointer = "autofire_enable",
    callback = function(state)
        getgenv().SKEGG_CONFIG.AutoFire.Enabled = state
        setup_auto_fire()
        
        if state and script_table.character_connection == nil then
            script_table.character_connection = local_player.CharacterAdded:Connect(function()
                task.wait(1)
                if getgenv().SKEGG_CONFIG.AutoFire.Enabled then
                    setup_auto_fire()
                end
...
AutoFireMainSection:Toggle({
    ...
    Default = true,
    Pointer = "autofire_activetool",
    callback = function(state)
        getgenv().SKEGG_CONFIG.AutoFire.AutoActivateTool = state
    end
})

FireRateSection:Slider({
    Name = "Fire Rate",
    Minimum = 1,
    Maximum = 30,
    Default = 10,
    Pointer = "autofire_rate",
    callback = function(value)
        getgenv().SKEGG_CONFIG.AutoFire.FireRate = value
    end
})

library:Initialize()

local watermark = library:Watermark({"skegg-agent", "v2.0.0", "FPS: 60"})
...
task.spawn(function()
    ...
    while task.wait(1) do
        watermark:Update({"skegg-agent", "v2.0.0", "FPS: " .. frames})
        frames = 0
    end
end)
...
local function get_humanoid_root_part(character)
    if character then
...
local function is_friend(player)
    if not getgenv().GLOW_CHAMS_CONFIG.FriendCheck then return true end
    return local_player:IsFriendsWith(player.UserId)
end

local function apply_glow_chams(player, character)
    if not character then return end
    ...
    local connection = run_service.Heartbeat:Connect(function()
        ...
        local root = get_humanoid_root_part(character)
    ...
    glow_data.connection = connection
end
...
local function toggle_glow_chams(state)
...
print("Prediction:", getgenv().SKEGG_CONFIG.SilentAim.Prediction)
print("Auto Fire Enabled:", getgenv().SKEGG_CONFIG.AutoFire.Enabled)
print("Auto Fire Rate:", getgenv().SKEGG_CONFIG.AutoFire.FireRate)
