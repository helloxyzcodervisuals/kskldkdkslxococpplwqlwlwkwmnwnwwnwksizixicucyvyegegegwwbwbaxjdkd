...
function library:ChangeThemeOption(option, color)
    ...
    for obj, theme in next, themeobjects do
        if obj.Parent and theme == option then
            if obj:IsA("GuiObject") then
...
function library.createkeybindlist()
    ...
    local list_outline = utility.create("Frame", {
        Visible = true,
    ...
    local list_inline = utility.create("Frame", {
        Parent = list_outline,
        Visible = true,
        BackgroundTransparency = 0,
    ...
    local list_accent = utility.create("Frame", {
        Parent = list_inline,
        Visible = true,
        BackgroundTransparency = 0,
        Theme = "Accent",
    ...
    local list_title = utility.create("TextLabel", {
        Text = "Keybinds",
        Parent = list_inline,
        Visible = true,
    ...
    local list_content = utility.create("Frame", {
        Parent = list_outline,
        Visible = true,
        BackgroundTransparency = 1,
    ...
    uiListLayout.Parent = list_content
    
    function keybind_list_tbl:add_keybind(name, key)
        local key_settings = {}
        
        local key_holder = utility.create("Frame", {
            Parent = list_content,
        ...
        function key_settings:is_active(state)
            if state then
                list_title.TextColor3 = library.theme["Accent"]
            else
                list_title.TextColor3 = library.theme["Text"]
            end
        end
...
function library.createcolorpicker(default, parent, count, flag, callback, offset)
    ...
    local copy = utility.create("Frame", {
        BackgroundTransparency = 0,
        BorderSizePixel = 1,
        Theme = "Object Background",
    ...
    local paste = utility.create("Frame", {
        BackgroundTransparency = 0,
        BorderSizePixel = 1,
        Theme = "Object Background",
        Size = UDim2.new(0.5, -20, 0, 12),
    ...
    utility.connect(copy.InputBegan, function(input)
    ...
    utility.connect(paste.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
...
function library.createdropdown(holder, content, flag, callback, default, max, scrollable, scrollingmax, islist, size, section, sectioncontent)
    ...
    local function createoption(name)
        ...
        if scrollable then
            if count < scrollingmax and not islist then
                contentframe.Size = UDim2.new(1, 0, 0, contentholder.AbsoluteContentSize.Y + 6)
            end
        else
            if not islist then
                contentframe.Size = UDim2.new(1, 0, 0, contentholder.AbsoluteContentSize.Y + 6)
                ...
                if max then
                    if table.find(chosen, option) then
                        ...
                        for _, opt in next, chosen do
                            ...
                            if utility.textlength(table.concat(textchosen, ", ") .. ", ...", Enum.Font.Code, 13).X > (dropdown.AbsoluteSize.X - 18) then
                                cutobject = true
                        ...
                        if #chosen == max then
                            local firstOpt = chosen[1]
                            if optioninstances[firstOpt] then
                                optioninstances[firstOpt].text.TextColor3 = library.theme["Text"]
                            end
    ...
    set = function(option)
        if max then
            ...
            for _, opt in next, chosen do
                ...
                if utility.textlength(table.concat(textchosen, ", ") .. ", ...", Enum.Font.Code, 13).X > (dropdown.AbsoluteSize.X - 6) then
                    cutobject = true
                    table.remove(textchosen, #textchosen)
                end
            end
    ...
    function dropdowntypes:remove(option)
        if optioninstances[option] then
            ...
            if scrollable then
                contentframe.Size = UDim2.new(1, 0, 0, math.clamp(contentholder.AbsoluteContentSize.Y, 0, (scrollingmax * 16) + ((scrollingmax - 1) * 3)) + 6)
            else
                contentframe.Size = UDim2.new(1, 0, 0, contentholder.AbsoluteContentSize.Y + 6)
            end
    ...
    box.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            if not connection then
                connection = utility.connect(UserInputService.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        if input.KeyCode ~= Enum.KeyCode.Backspace then
                            local str = UserInputService:GetStringForKeyCode(input.KeyCode)

                            if table.find(allowedcharacters, str) then
                                keyqueue = keyqueue + 1
                                local currentqueue = keyqueue

                                if not UserInputService:IsKeyDown(Enum.KeyCode.RightShift) and not UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) then
                                    ...
                                    coroutine.wrap(function()
                                        ...
                                        while UserInputService:IsKeyDown(input.KeyCode) and currentqueue == keyqueue  do
                                            text.Text = text.Text .. str:lower()
                                            callback(text.Text)
                ...
                backspaceconnection = utility.connect(UserInputService.InputBegan, function(input)
...
function library:new_window(cfg)
    ...
    local window_drag = utility.create("Frame", {
        Parent = window_outline,
        Visible = true,
        BackgroundTransparency = 1,
        Size = UDim2.new(1,0,0,10),
    ...
    local dragoutline = utility.create("Frame", {
        Size = UDim2.new(0, size_x, 0, size_y),
    ...
    function window_tbl:new_page(cfg)
        ...
        function page_tbl:new_section(cfg)
            ...
            local section_title_cover = utility.create("Frame", {
                ...
                BackgroundTransparency = 0,
                Theme = "Window Holder Background",
                Size = UDim2.new(0,utility.textlength(section_name, Enum.Font.Code, 13).X + 2,0,4),
            ...
            function section_tbl:new_slider(cfg)
                ...
                local slider_drag = utility.create("Frame", {
                    Parent = slider_frame,
                    Visible = true,
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1,0,1,0),
                        ...
                        task.spawn(function()
                            if button_confirm then
                                if clicked then
                                    clicked = false
                                    counting = false
                                    button_title.TextColor3 = library.theme["Text"]
                                    ...
                                    for i = 3,1,-1 do
                                        ...
                                        button_title.TextColor3 = library.theme["Accent"]
                                        wait(1)
                                    end
                                    clicked = false
                                    counting = false
            ...
            function section_tbl:new_keybind(cfg)
                ...
                local holdbutton = utility.create("Frame", {
                    BackgroundTransparency = 1,
                ...
                local remove = utility.create("TextLabel", {
                    Font = Enum.Font.Code,
                    TextSize = 13,
                ...
                utility.connect(UserInputService.InputBegan, function(inp)
                ...
                frame.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        if not binding then
                            ...
                            binding = utility.connect(UserInputService.InputBegan, function(input, gpe)
            ...
            function section_tbl:new_seperator(cfg)
                ...
                local sizeX = utility.textlength(seperator_text, Enum.Font.Code, 13).X
            ...
            function section_tbl:new_textbox(cfg)
                ...
                local text = utility.create("TextLabel", {
                    ...
                    Font = Enum.Font.Code,
                    TextSize = 13,
                    AnchorPoint = middle and Vector2.new(0.5, 0) or Vector2.new(0, 0),
        ...
        function page_tbl:new_section_holder(cfg)
            ...
            local section_button_holder = utility.create("Frame", {
                BackgroundTransparency = 1,
            ...
            function section_tbl:new_section(cfg)
                ...
                local multi_title = utility.create("TextLabel", {
                    Text = name,
                    Parent = multi_button,
                    Visible = true,
                    BackgroundTransparency = 1,
                ...
                local multi_button_accent = utility.create("Frame", {
                    Parent = multi_button,
                    Visible = false,
                    BackgroundTransparency = 0,
                    Theme = "Accent",
                ...
                utility.connect(multi_button.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        multi_button.BackgroundColor3 = library.theme["Page Selected"]
