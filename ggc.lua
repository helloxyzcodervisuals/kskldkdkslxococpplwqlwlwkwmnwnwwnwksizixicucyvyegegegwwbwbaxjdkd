...
function library:addDropdown(options) 
    ...
    function cfg.refreshOptions(list) 
        ...
        for _, option in next, list do 
            ...
            OPTION_INSTANCE.MouseButton1Click:Connect(function()
                if cfg.multi then 
...
function library:addList(options)
    ...
    function cfg.set(value)
        if type(value) ~= "string" then
            warn("Listbox set expects string, got " .. type(value))
            return
        end
    ...
    function cfg.addItem(text)
        if type(text) ~= "string" then
            warn("addItem expects string, got " .. type(text))
            return
        end
        if not find(cfg.items, text) then
            insert(cfg.items, text)
            cfg.refresh_options(cfg.items)
        end
    end

    function cfg.removeItem(text)
        if type(text) ~= "string" then
            warn("removeItem expects string, got " .. type(text))
            return
        end
        local index = find(cfg.items, text)
        if index then
            remove(cfg.items, index)
            cfg.refresh_options(cfg.items)
        end
    end
